    ...
    public class RapidDevelopmentBusiness : BaseBusiness<Base_DatabaseLink>
    {
        ...
        public void BuildCode(string linkId, string areaName, string tables, string buildType)
        {
            ...
            tableList.ForEach(aTable =>
            {
                ...
                {
                    BuildBusinessInterface(areaName, aTable);
...
{{
    ...
    {{
        ...
        #region 数据模型
            ...
            string businessPath = _contentRootPath.Replace("CoreFrame.Web", "CoreFrame.Business");
        ...
        /// <param name="areaName">区域名</param>
        /// <param name="entityName">实体名</param>
        private void BuildBusinessInterface(string areaName, string entityName)
        {
            string code = code =
...
{{
    ...
    {{
        #region 外部接口
            ...
            string businessPath = _contentRootPath.Replace("CoreFrame.Web", "CoreFrame.Business");
            string filePath = Path.Combine(businessPath, areaName, $"I{entityName}Business.cs");
...
{{
    ...
    {{
        private I{entityName}Business {varBusiness} ;

        public {entityName}Controller(I{entityName}Business dev_{entityName}Business)
        {{
            {varBusiness} = dev_{entityName}Business;
        }}
        ...
        {{
            if(theData.Id == 0)
            {{
                {varBusiness}.Insert(theData);
            }}
            else
            {{
                {varBusiness}.Update(theData);
            }}
        ...
        {{
            {varBusiness}.Delete(ids.ToList<string>());
    ...
    $(function () {{
        ...
        $('#add').click(function () {{
            dialogOpen({{
                ...
                width: formWidth,
                height: formHeight,
                url: rootUrl + '{areaName}/{entityName}/Form?id=0',
            }});
        }});
