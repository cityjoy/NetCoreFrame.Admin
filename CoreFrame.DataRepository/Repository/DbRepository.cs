...
namespace CoreFrame.DataRepository
{
    ...
    public class DbRepository : IRepository
    {
        ...
        public int Delete<T>(List<int> keys) where T : class, new()
        {
            var theProperty = GetKeyProperty<T>();
            if (theProperty == null)
                throw new Exception("该实体没有主键标识！请使用[Key]标识主键！");
        ...
        /// <returns></returns>
        public IQueryable<T> GetIQueryableList<T>(Expression<Func<T, bool>> predicate = null) where T : class, new()
        {
        ...
        public int ExecuteSql(string sql, List<DbParameter> parameters)
        {
            ...
            if (!_openedTransaction)
            {
                result = Db.Database.ExecuteSqlCommand(sql, parameters.ToArray());
                Dispose();
            }
            else
            {
