...
namespace CoreFrame.Util
{
    ...
    public class StringHelper
    {
        ...
        {
            return Regex.IsMatch(input, @"^[A-Za-z\d]+([-_.][A-Za-z\d]+)*@([A-Za-z\d]+[-.])+[A-Za-z\d]{2,4}$ ", RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace);
        }
        ...
        public static string CreateLinkQueryString(NameValueCollection collection, Encoding encoding)
        {
            ...
            foreach (string key in collection.Keys)
            {
                ...
                stb.AppendFormat("{0}={1}&", HttpUtility.UrlEncode(key), HttpUtility.UrlEncode(value));
            }
            if (stb.Length > 0)
            {
